/*
  # Trade Proposal System Migration

  1. New Tables
    - `trade_proposals` table to track trade proposal lifecycle
    - `shipping_preferences` table to store user shipping addresses

  2. Features
    - Status tracking (proposed, accepted, confirmed, shipping, etc.)
    - Shipping method selection (mail vs local meetup)
    - Automatic status transitions via triggers
    - Transaction creation on trade confirmation
...
ALTER TABLE trade_proposals DROP CONSTRAINT IF EXISTS check_proposal_status;
...
ALTER TABLE trade_proposals DROP CONSTRAINT IF EXISTS check_shipping_method;
...
DROP POLICY IF EXISTS "Users can view their trade proposals" ON trade_proposals;
DROP POLICY IF EXISTS "Users can insert their own trade proposals" ON trade_proposals;
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can view their shipping preferences" ON shipping_preferences;
DROP POLICY IF EXISTS "Users can insert their shipping preferences" ON shipping_preferences;
DROP POLICY IF EXISTS "Users can update their shipping preferences" ON shipping_preferences;
DROP POLICY IF EXISTS "Users can delete their shipping preferences" ON shipping_preferences;
...
-- Add function to update trade proposal status
CREATE OR REPLACE FUNCTION update_trade_proposal_status() RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger for trade_proposals
...
-- Add function to create transaction record when trade is confirmed
CREATE OR REPLACE FUNCTION create_transaction_for_confirmed_trade() RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger for creating transactions
...
-- Add function to update match status when trade proposal status changes
CREATE OR REPLACE FUNCTION update_match_status_from_proposal() RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger for updating match status
...
-- Add function to prevent duplicate trade proposals
CREATE OR REPLACE FUNCTION prevent_duplicate_trade_proposals() RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger to prevent duplicate proposals
