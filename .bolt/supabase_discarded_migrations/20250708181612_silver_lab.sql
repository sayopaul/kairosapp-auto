/*
  # Add Trade Proposals System

  1. New Tables
    - `trade_proposals` table to track trade proposal status and confirmations
    - `shipping_preferences` table to store user shipping addresses
    
  2. Functions and Triggers
    - Functions to manage trade proposal lifecycle
    - Automatic status updates based on user actions
    - Prevent duplicate proposals for the same match
...
BEGIN
  SELECT EXISTS (
    ...
    WHERE proname = 'update_trade_proposal_status'
...
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'create_transaction_for_confirmed_trade'
  ) INTO function_exists;
...
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'update_match_status_from_proposal'
  ) INTO function_exists;
...
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'prevent_duplicate_trade_proposals'
  ) INTO function_exists;
