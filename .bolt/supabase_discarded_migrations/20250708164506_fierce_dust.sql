/*
  # Trade Confirmation Flow Database Structure

  1. New Tables
    - `trade_proposals` table to track trade proposals and their status
    - `shipping_preferences` table to store user shipping addresses
    
  2. Schema
    - trade_proposals: Tracks the entire lifecycle of a trade from proposal to completion
    - shipping_preferences: Stores user shipping addresses for reuse
    
  3. Functions and Triggers
    - Auto-update trade status based on user confirmations
    - Create transaction records when trades are confirmed
    - Update match status when trade status changes
    - Prevent duplicate trade proposals
*/
...
CREATE INDEX idx_trade_proposals_match_id ON trade_proposals(match_id);
...
-- Create shipping_preferences table
...
CREATE INDEX idx_shipping_preferences_user_id ON shipping_preferences(user_id);
