...
import { Plus, Upload, Search, Crown, FileText } from 'lucide-react';
...
const Want: React.FC = () => {
  ...
  const { cards: wantCards, loading, deleteCard, refetch } = useCards(user?.id, 'want');
  ...
  const totalValue = wantCards.reduce((sum, card) => sum + (card.market_price * card.quantity), 0);
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-2xl p-8 text-white relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-3xl font-bold mb-2">Your Want List</h1>
          <p className="text-gray-300 text-lg">Cards you're looking to acquire</p>
          <div className="mt-4 flex items-center space-x-6">
            <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2 border border-white/20">
              <span className="text-sm text-gray-300">Wanted Cards:</span>
              <span className="ml-2 text-xl font-bold">{wantCards.length}</span>
              {limits.maxWantCards !== Infinity && (
                <span className="text-sm text-gray-400">/{limits.maxWantCards}</span>
              )}
            </div>
            <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2 border border-white/20">
              <span className="text-sm text-gray-300">Target Value:</span>
      ...
      {limits.maxWantCards !== Infinity && wantCards.length >= limits.maxWantCards * 0.9 && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <div className="flex items-start space-x-3">
            <Crown className="h-5 w-5 text-orange-600 mt-0.5" />
            <div>
              <h3 className="font-semibold text-orange-900">
                {wantCards.length >= limits.maxWantCards ? 'Want List Limit Reached' : 'Approaching Want List Limit'}
              </h3>
              <p className="text-orange-800 text-sm mt-1">
                You have {wantCards.length} of {limits.maxWantCards} want cards. 
                {wantCards.length >= limits.maxWantCards 
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        >
          ...
          <span className="font-medium">Add Wanted Card</span>
        </button>
        ...
        <button className="flex items-center justify-center space-x-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all duration-200 shadow-lg hover:shadow-xl">
      ...
      {wantCards.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">No cards on your want list</h3>
          <p className="text-gray-600 mb-4">Add cards you're looking for to help find trading opportunities</p>
          <button 
            onClick={() => setShowAddCard(true)}
