...
import { X, ArrowLeftRight, Check, Clock, MapPin, Truck, MessageCircle, AlertCircle, CheckCircle, Loader2 } from 'lucide-react';
...
const TradeProposalModal: React.FC<TradeProposalModalProps> = ({
  ...
  matchId,
  matchScore,
  user1 = { id: '', username: 'User 1' },
  user2 = { id: '', username: 'User 2' },
  user1Card,
  user2Card,
  isBundle = false,
  user1Cards = [],
  ...
  const { shippingPreferences, getDefaultShippingPreference } = useShippingPreferences(user?.id);
  ...
  const [enhancedPricing, setEnhancedPricing] = useState<{
    myCard?: { price: number; source: string };
    theirCard?: { price: number; source: string };
  }>({});

  // // Fetch enhanced pricing on component mount
  // // Disable fetching enhanced pricing
  // React.useEffect(() => {
  //   const fetchEnhancedPricing = async () => {
  //     const results: any = {};
      
  //     // Get pricing for single cards
  //     if (!isBundle && myCard && theirCard) {
  //       try {
  //         const [myCardPricing, theirCardPricing] = await Promise.all([
  //           JustTcgApiService.updateCardPricing({
  //             name: myCard.name,
  //             set: myCard.set,
  //             condition: myCard.condition
  //           }),
  //           JustTcgApiService.updateCardPricing({
  //             name: theirCard.name,
  //             set: theirCard.set,
  //             condition: theirCard.condition
  //           })
  //         ]);
          
  //         if (myCardPricing) {
  //           results.myCard = { price: myCardPricing.price, source: 'JustTCG' };
  //         }
  //         if (theirCardPricing) {
  //           results.theirCard = { price: theirCardPricing.price, source: 'JustTCG' };
  //         }
  //       } catch (error) {
  //         console.warn('Failed to fetch enhanced pricing:', error);
  //       }
  //     }
      
  //     setEnhancedPricing(results);
  //   };

  //   fetchEnhancedPricing();
  // }, [myCard?.name, theirCard?.name, isBundle]);

  // Validate required data
  const isValidData = () => {
    if (!matchId || !user1?.id || !user2?.id) {
      console.error('Missing required match data:', { matchId, user1: user1?.id, user2: user2?.id });
      return false;
    }
    
    if (!isBundle && (!user1Card || !user2Card)) {
      console.error('Missing card data for single trade:', { user1Card: !!user1Card, user2Card: !!user2Card });
      return false;
    }
    
    if (isBundle && (!user1Cards?.length || !user2Cards?.length)) {
      console.error('Missing card data for bundle trade:', { user1Cards: user1Cards?.length, user2Cards: user2Cards?.length });
      return false;
    }
  ...
  useEffect(() => {
    if (!isValidData()) {
      setMessage({
        type: 'error',
        text: 'Invalid trade match data. Please try again.'
      });
      return;
    }
    ...
    if (status === 'proposed') {
      ...
      console.log('IT IS SENDING STATUS AS COMPLETED');
      setStep('complete');
    }
  ...
  useEffect(() => {
    const defaultAddress = getDefaultShippingPreference();
    if (defaultAddress) {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      await updateProposal(existingProposal.id, { status: 'accepted_by_recipient' });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await updateProposal(existingProposal.id, { status: 'confirmed' });
  ...
  const renderCardDetails = (card: Card | undefined) => {
    ...
    const isMyCard = card === myCard;
    const enhancedPrice = isMyCard ? enhancedPricing.myCard : enhancedPricing.theirCard;
    const displayPrice = enhancedPrice?.price || parseFloat(card.market_price.toString());
    const priceSource = enhancedPrice?.source || 'Database';
  ...
  const renderBundleCards = (cards: Card[]) => {
    ...
    // For bundles, use database prices for now (could be enhanced later)
    const totalValue = cards.reduce((sum, card) => sum + parseFloat(card.market_price.toString()), 0);
        ...
        <div className="flex flex-wrap gap-3">
          {cards.map(card => (
            <div key={card.id} className="flex flex-col items-center w-20">
              ...
              <span className="text-xs text-gray-500">${parseFloat(card.market_price.toString()).toFixed(2)}</span>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
          <div className="bg-gradient-to-r from-red-600 to-red-700 p-6 rounded-t-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Invalid Trade Data</h2>
                  <p className="text-red-100">Unable to load trade information</p>
                </div>
              </div>
              <button
                onClick={onClose}
          ...
          <div className="p-6">
            <div className="flex items-center space-x-3 p-4 rounded-lg border bg-red-50 border-red-200 text-red-800 mb-6">
              <AlertCircle className="h-5 w-5 flex-shrink-0" />
              <span className="font-medium">Invalid trade match data. Please try again.</span>
            </div>
            <button
              onClick={onClose}
                  ...
                  {showAddressForm ? (
                    ...
                    <div className="mb-6">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        >
                          {shippingPreferences.length > 0 ? 'Add New Address' : 'Add Address'}
                        </button>
                      </div>
