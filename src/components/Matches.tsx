...
const Matches: React.FC<MatchesProps> = ({ onTabChange = () => {} }) => {
  ...
  const matches = useGeneratedMatches && generatedMatches.length > 0 ? generatedMatches : savedMatches;
    ...
    return (
      <div className="flex items-center justify-center min-h-96">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-2">{matchingStatus || 'Analyzing potential trades...'}</p>
          {matchingProgress > 0 && (
            <div className="w-64 mx-auto">
              ...
              <p className="text-sm text-gray-500 mt-2">{Math.round(matchingProgress)}% complete</p>
            </div>
          )}
        </div>
      </div>
      ...
      {(tradeCards.length === 0 || wantCards.length === 0) && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 md:p-6">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="font-semibold text-yellow-900 mb-2">Setup Required for Matching</h3>
              <p className="text-yellow-800 mb-4">
                To find trading matches, you need both cards to trade and cards you want:
              </p>
      ...
      {matchingError && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 md:p-6">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="font-semibold text-red-900 mb-2">Matching Information</h3>
      ...
      {filteredMatches.length > 0 && (
        <div className="space-y-6">
          {filteredMatches.map((match) => {
            ...
            if (user && match) {
              if (match.user1_id && match.user1_id === user.id) isCurrentUserUser1 = true;
              else if (match.user2_id && match.user2_id === user.id) isCurrentUserUser1 = false;
              else if (match.user1 && match.user1.id === user.id) isCurrentUserUser1 = true;
              else if (match.user2 && match.user2.id === user.id) isCurrentUserUser1 = false;
                    ...
                    <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-1">
                              <p className="text-lg font-bold text-blue-600">
                                ${(enhancedPricing[myCard.id]?.price || parseFloat(myCard.market_price || 0)).toFixed(2)}
                              </p>
                      ...
                      {isBundle && (
                        <div className="grid grid-cols-3 sm:grid-cols-4 md:flex md:flex-wrap gap-2 md:gap-3">
                          ...
                          }) : <span className="text-sm text-gray-400">Card details unavailable</span>}
                        </div>
                      )}
                    </div>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-1">
                              <p className="text-lg font-bold text-gray-600">
                                ${(enhancedPricing[theirCard.id]?.price || parseFloat(theirCard.market_price || 0)).toFixed(2)}
                              </p>
                  ...
                  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 mt-4">
                    {existingProposal ? (
                      ...
                      >
                        ...
                        <span className="font-medium">
                          {existingProposal?.status === 'proposed' && existingProposal.recipient_id === user?.id
                            ? 'Review Proposal'
                            : existingProposal?.status === 'accepted_by_recipient' && existingProposal.proposer_id === user?.id
                            ? 'Confirm Trade'
