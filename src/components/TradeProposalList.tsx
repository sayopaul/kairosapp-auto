...
const TradeProposalList: React.FC = () => {
  ...
  const { user } = useAuth();
  const { proposals, loading, error, refetchProposals } = useTradeProposals(user?.id);
  ...
  const stats = [
    ...
    { 
      label: 'Accepted', 
      value: proposals.filter(p => p.status === 'accepted_by_recipient').length,
      icon: Check,
      color: 'bg-green-100 text-green-600'
    },
    { 
      label: 'In Progress', 
      value: proposals.filter(p => p.status === 'confirmed' || p.status === 'shipping_pending').length,
      icon: RefreshCw,
    ...
    { 
      label: 'Declined', 
      value: proposals.filter(p => p.status === 'declined' || p.status === 'cancelled').length,
      icon: X,
  ...
  const getLatestStatus = (proposal: TradeProposal): string => {
    if (proposal.status === 'proposed') {
      ...
      if (proposal.proposer_shipping_confirmed && proposal.recipient_shipping_confirmed) {
        return 'completed';
      } else {
  ...
  const filteredProposals = proposals.filter(proposal => {
    ...
    if (!match) {
      console.warn('🚨 Found proposal with no match data (should have been deleted):', proposal.id);
      return false;
    }
    ...
    if (!isBundle) {
      ...
      const hasUser1Card = match.user1_card || (match.user1_cards && match.user1_cards.length > 0);
      const hasUser2Card = match.user2_card || (match.user2_cards && match.user2_cards.length > 0);
      
      if (!hasUser1Card || !hasUser2Card) {
        console.warn('🚨 Found proposal with missing single trade card data (should have been deleted):', proposal.id);
        return false;
      }
    } else {
      ...
      if (!hasUser1Cards || !hasUser2Cards) {
        console.warn('🚨 Found proposal with missing bundle card data (should have been deleted):', proposal.id);
        return false;
      }
    }
  ...
  const renderCardDetails = (card: any) => {
    ...
    return (
      <div className="flex items-center space-x-3">
        ...
        />
        <div>
          <h5 className="font-semibold text-gray-900">{card.name || 'Unknown Card'}</h5>
          <p className="text-sm text-gray-600">
            {card.card_number ? `#${card.card_number}` : ''} 
            {card.set ? ` • ${card.set}` : ''}
          </p>
          <p className="text-sm font-bold text-blue-600">
            ${card.market_price ? parseFloat(card.market_price).toFixed(2) : '0.00'}
          </p>
        </div>
      </div>
    );
  ...
  const renderBundleCards = (cards: any[]) => {
    ...
    const totalValue = cards.reduce((sum, card) => sum + (parseFloat(card.market_price) || 0), 0);
    
    return (
      <div className="space-y-2">
        <div className="flex flex-wrap gap-2">
          {cards.slice(0, 3).map((card, index) => (
            <div key={card.id || index} className="flex flex-col items-center w-16">
              <img
                src={card.image_url || 'https://images.pexels.com/photos/7708408/pexels-photo-7708408.jpeg?auto=compress&cs=tinysrgb&w=300&h=400&dpr=2'}
                alt={card.name || 'Card'}
        ...
        <div className="text-right">
          ...
          <span className="ml-1 font-bold text-blue-600">${totalValue.toFixed(2)}</span>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="font-semibold text-red-900">Error Loading Proposals</h3>
...
export default TradeProposalList;